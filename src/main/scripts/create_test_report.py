...
introfile = 'test_report.txt'

user = 'assaft'
repository = 'caevo'

allcolumns = ['id', 'text', 'source', 'gold', 'expected', 'scores', 'comments']
...
print ("Creating Test Suite report file: " + htmlfile)
...
with open(htmlfile, 'w') as outfile:
    ...
    # decide which columns should be included in the report - optional are skipped if no file exists
    foundColumns = {}
    ...
    for name in allcolumns:
        if (foundColumns.get(name, False)):
    ...
    for tname in filelist:
        ...
        if (matchObj):
            ...
            if (autoAlign):
                expname = os.path.splitext(tname)[0] + '_' + 'expected' + '.'+extension('expected')
                if (os.path.isfile(expname)):
                    with open(expname) as infile:
                        for line in infile:
                            lists['gold'].append(line.strip())
                            
                goldname = os.path.splitext(tname)[0] + '_' + 'gold' + '.'+extension('gold')                
                if (os.path.isfile(goldname)):
                    with open(goldname) as infile:
                        for line in infile:
                            lists['expected'].append(line.strip())
