...
public class ClosureTest extends TestCase {
	...
	public void testClosure() throws Exception {
		...
		for( TLink link : newClosed )
			assertTrue("Checking generated link (" + link + ") against correct list", Evaluate.isLinkCorrect(link, expectedLinks));

		// Check that each expected link appears in the closed list!
		for( TLink link : expectedLinks )
			assertTrue("Didn't find (" + link + ") in the auto-generated closure list.", Evaluate.isLinkCorrect(link, newClosed));		
	}
